...
from isaaclab.sensors import TiledCamera, TiledCameraCfg, save_images_to_file, ContactSensor, ContactSensorCfg
...
# from isaacsim.core import PhysicsContext
...
class MyBuddyEnvCfg(DirectRLEnvCfg):
    ...
    sim: SimulationCfg = SimulationCfg(dt=1 / 60, render_interval=decimation,
                                       physx=PhysxCfg(solver_type=1, enable_ccd=True, gpu_temp_buffer_capacity=2**26),
                                       gravity=(0.0, 0.0, 9.81),
                                       render=RenderCfg(enable_dlssg=True))
    ...
    # arm_dof_name = ["left_arm_j1", "left_arm_j2", "left_arm_j3", "left_arm_j4"]
    ...
    observation_space = {"joints": gym.spaces.Box(low=-20.00, high=20.0, shape=(len(arm_dof_name),), dtype=np.float32),
                         "rgb": gym.spaces.Box(low=-1.0, high=1.0, shape=(tiled_camera.height, tiled_camera.width, 4), dtype=np.float32), 
...
class MyBuddyEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: MyBuddyEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.root_init_angles = torch.deg2rad(torch.tensor([0, -90, 0, 120, -120, 180], device=self.device)).repeat(self.num_envs, 1).view(-1, len(self.cfg.arm_dof_name)) #torch.deg2rad(torch.tensor([-90, 0, 120, -120], device=self.device)).repeat(self.num_envs, 1).view(-1, len(self.cfg.arm_dof_name))
        ...
        self.cube_maskas = torch.zeros((self.num_envs, self.cfg.tiled_camera.width, self.cfg.tiled_camera.height, 1), device=self.device, dtype=torch.float32)
        self.rgb_image = torch.zeros((self.num_envs, self.cfg.tiled_camera.width, self.cfg.tiled_camera.height, 3), device=self.device, dtype=torch.float32)
    ...
    def _pre_physics_step(self, actions: torch.Tensor) -> None:
        ...
        new_actions = torch.clamp(actions.clone(), -1, 1) * self.dt * self.action_scale
        # Increment angles based on previous state
        self.actions = new_actions + self.last_angles
        self.actions[:, 0] = torch.clamp(self.actions[:, 0], -0.5, 0.5)
        self.actions[:, 1] = torch.clamp(self.actions[:, 1], -1.91986, -1.39626)
        
        # Update current actions
        self.last_angles = self.actions
    ...
    def _get_observations(self) -> dict:
        ...
        # Observations
        obs = {"rgb":image_obs.to(dtype=torch.float32), "joints": joint_readings.to(dtype=torch.float32), "ee_position": self.ee_position.to(dtype=torch.float32)}
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        fruit_reward, plant_mask = calculate_cube_reward(depth_image.clone())
        ...
        self.done = torch.where(occlusion_pixels <= 20, torch.tensor(True, device=self.device), torch.tensor(False, device=self.device)).reshape(self.num_envs)
    ...
    #     # Occlusion Reward
    #     occlusion_mask = self.cube_maskas * plant_mask
    #     # save_images_to_file(self.cube_maskas * 255, "/home/nitesh/IsaacLab/mybuddy_cube_mask.jpg")
    #     # save_images_to_file(occlusion_mask * 255, "/home/nitesh/IsaacLab/mybuddy_occlusion_mask.jpg")
    #     occlusion_pixels = torch.sum(occlusion_mask, dim=(1, 2, 3)).reshape(-1, 1)
    #     occlusion_reward = (-occlusion_pixels / (80*80)) * 2.0
    ...
    #     sustained_reward = sustained_detection.float() * 100.0  # High reward
    ...
    #     occlusion_reward = occlusion_reward.reshape(self.num_envs)
    #     total_reward = (contact_reward + occlusion_reward + ee_pos_reward + ee_pos_done_reward + fruit_reward + full_visibility_reward + sustained_reward)  # Add the penalty here
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        if np.random.uniform() < 0.7:
            ...
            joint_pos[:, 0] = sample_uniform(-0.3, 0.3, joint_pos[:, 0].shape, self.device)
        ...
        cube_masks = torch.flip(create_batched_masks((self.cfg.tiled_camera.width, self.cfg.tiled_camera.height), image_points_2d, 80), dims=[2]).to(dtype=torch.float32, device=self.device)
...
@torch.jit.script
def calculate_cube_reward(depth_obs):
    fruit_mask = ((depth_obs >= 0.35) & (depth_obs <= 0.54)).clone().detach()/255#.to(dtype=torch.float32, device=device) / 255
    plant_mask = ((depth_obs >= 0.01) & (depth_obs <= 0.35)).clone().detach()/255#.to(dtype=torch.float32, device=device) / 255
    ...
    fruit_reward = (fruit_pixels / (80*80))
